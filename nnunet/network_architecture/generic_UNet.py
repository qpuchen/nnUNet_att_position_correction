...
class Generic_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 encoder_scale=1, axial_attention=True, heads=8, dim_heads=32, volume_shape=[112, 160, 128], no_attention=[0, 2, 3, 4], dropout_level=[4]): #extra
        """
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            ...
            if self.do_attention and u not in self.no_attention:
                x = self.axial_attention[u-1](self.axial_embedding[u-1](x)) + x
...
class AxialAttention3D(nn.Module):
    def __init__(self, n_features, emb_shape, heads, dim_heads, non_lin, non_lin_kwargs, bn=True, sum_axial_out=True, residual_attention=False):
        super(AxialAttention3D, self).__init__()
        self.axial_attention = [AxialPositionalEmbedding(dim=n_features,shape=emb_shape),
                                AxialAttention(dim = n_features,
                                                heads = heads,
                                                dim_heads = dim_heads,
                                                dim_index = 1,
                                                num_dimensions = 3,
...
class Axial_UNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 max_num_features=None, basic_block=ConvDropoutNormNonlin,
                 seg_output_use_bias=False, 
                 encoder_scale=1, axial_attention=False, heads=2, dim_heads=8, volume_shape=(128,128,128), no_attention=[0], axial_bn=True, sum_axial_out=True, residual_attention=False): #extra
        """
        basically more flexible than v1, architecture is the same
        ...
        for d in range(num_pool):
            ...
            if self.do_attention:
                if d not in self.no_attention:
                    emb_shape = (self.volume_shape/(2**d)).astype(np.int16)
                    self.axial_attention_down.append(AxialAttention3D(n_features = output_features, 
                                                                      emb_shape  = emb_shape,
                                                                      heads = heads*2**d,
                                                                      dim_heads = dim_heads*2**d,
                                                                      non_lin = nonlin,
                                                                      non_lin_kwargs = nonlin_kwargs,
                                                                      bn = axial_bn,
                                                                      sum_axial_out = sum_axial_out,
        ...
        for u in range(num_pool):
            ...
            if self.do_attention:
                ...
                if d not in self.no_attention:
                    emb_shape = (self.volume_shape/(2**d)).astype(np.int16)
                    self.axial_attention_up.append(AxialAttention3D(n_features = nfeatures_from_skip, 
                                                                      emb_shape  = emb_shape,
                                                                      heads = heads*2**d,
                                                                      dim_heads = dim_heads*2**d,
                                                                      non_lin = nonlin,
